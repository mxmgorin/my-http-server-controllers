...
impl DataTypeProvider for u8 {
    fn get_data_type(_generic_type: Option<&'static str>) -> HttpDataType {
...
impl<T: DataTypeProvider> DataTypeProvider for Vec<T> {
    fn get_data_type(generic_type: Option<&'static str>) -> HttpDataType {
        let data_type = T::get_data_type(generic_type);
        match data_type {
...
impl<TValue: DataTypeProvider> DataTypeProvider for HashMap<String, TValue> {
    fn get_data_type(generic_type: Option<&'static str>) -> HttpDataType {
        match TValue::get_data_type(generic_type) {
            ...
            _ => panic!(
                ...
                TValue::get_data_type(generic_type)
            ),
        }
    }
}
